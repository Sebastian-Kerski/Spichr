...
final class SettingsViewModel {
    ...
    var iCloudAccountStatus: String = NSLocalizedString("icloud_checking", comment: "")
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    switch status {
                    case .available:
                        iCloudAccountStatus = NSLocalizedString("icloud_connected", comment: "")
                    case .noAccount:
                        iCloudAccountStatus = NSLocalizedString("icloud_no_account", comment: "")
                    case .restricted:
                        iCloudAccountStatus = NSLocalizedString("icloud_restricted", comment: "")
                    case .temporarilyUnavailable, .couldNotDetermine:
                        iCloudAccountStatus = NSLocalizedString("icloud_not_available", comment: "")
                    @unknown default:
                        iCloudAccountStatus = NSLocalizedString("icloud_not_available", comment: "")
                    }
                }
                ...
                await MainActor.run {
                    iCloudAccountStatus = NSLocalizedString("icloud_error", comment: "")
                }
            }
        }
    }
    ...
    func exportData() async throws -> URL {
        try BackupService.shared.exportJSON()
    }

    func importData(from url: URL) async throws -> Int {
        try BackupService.shared.importJSON(from: url)
    }
