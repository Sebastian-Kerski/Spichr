...
final class StockListViewModel {
    private let logger = Logger(subsystem: "com.de.SkerskiDev.Spichr", category: "StockList")
    ...
    var filteredItems: [FoodItem] {
        ...
        if !searchQuery.isEmpty {
            result = result.filter {
                $0.unwrappedName.localizedCaseInsensitiveContains(searchQuery) ||
                $0.unwrappedLocation.localizedCaseInsensitiveContains(searchQuery) ||
                ($0.barcode?.contains(searchQuery) == true)
            }
        }
        ...
        case .noDate:
            result = result.filter { $0.effectiveExpirationDate == nil }
        }

        if !showExpiredItems {
            result = result.filter {
                guard let d = $0.daysUntilExpiration else { return true }
                return d >= 0
            }
        }
    ...
    func markAsConsumed(_ item: FoodItem) {
        repository.markAsConsumed(item)
        loadItems()
    }
    ...
    func confirmScannedProduct(
        ...
        await MainActor.run {
            ...
            let name = productInfo?.name ?? barcode
            let quantity = productInfo?.quantity ?? ""
            let store = productInfo?.brand
            let defaultLocation = NSLocalizedString("location_fridge", comment: "")
            ...
            logger.info("Product confirmed and saved: \(name, privacy: .private)")
    ...
    enum ExpiryFilter: String, CaseIterable, Identifiable {
        case all      = "all"
        case expiring7  = "7days"
        case expiring30 = "30days"
        case noDate   = "noDate"
        ...
        var label: String {
            switch self {
            case .all:       return NSLocalizedString("filter_expiry_all", comment: "")
            case .expiring7: return NSLocalizedString("expiring_7_days", comment: "")
            case .expiring30: return NSLocalizedString("expiring_30_days", comment: "")
            case .noDate:    return NSLocalizedString("group_no_date", comment: "")
            }
        }
    }
